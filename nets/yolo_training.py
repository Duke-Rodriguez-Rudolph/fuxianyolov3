    ...
    def init_func(m):
        ...
        if hasattr(m,'weight') and classname.find('Conv')!=-1:
            if init_type=='normal':
                ...
                raise NotImplementedError('无此初始化方法')
...
def jaccard(_box_a, _box_b):
    ...
    A = box_a.size(0)
    B = box_b.size(0)
    #上图以一点化四角，a为标签中的框，b为先验框，shape为[n,4]
    #多少个目标就弄成多少个匹配九个先验框的组
    #[target_size,anchor_num,2]
...
class yololoss(nn.Module):
    ...
    def get_target(self,target,anchors,in_w,in_h,ignore_threshold):
        ...
        for b in range(bs):
            ...
            anch_ious = jaccard(gt_box, anchor_shapes)#计算iou，实际上就算找出标签所框住的物体适用于哪个维度的，与其中的哪一个先验框
            #输出的东西为[targets_nums,anchor_nums]
            best_ns = torch.argmax(anch_ious,dim=-1)
            #一个列表，每个元素都说明了每个目标最大的iou在anch_ious中的哪个位置
            for i, best_n in enumerate(best_ns):
                ...
                if (gj < in_h) and (gi < in_w):
                    ...
                    noobj_mask[b, best_n, gj, gi] = 0#输出的是哪个大小的哪个点负责
